...
contract GiveToken is ERC20, ERC20Burnable, ERC20Permit, ERC20Votes, Ownable, ReentrancyGuard, Pausable, AccessControl {
    ...
    /**
     * @dev Transfer tokens from contract to specified address (only admin)
     * @param _to Address to receive tokens
     * @param _amount Amount of tokens to transfer
     * @notice Allows admin to transfer ICO tokens from contract for liquidity provision
     */
    function transferFromContract(address _to, uint256 _amount) external onlyRole(ADMIN_ROLE) {
        if (_to == address(0)) revert InvalidAddress();
        if (_amount == 0) revert InvalidBurnAmount();
        if (balanceOf(address(this)) < _amount) revert InsufficientContractBalance();
